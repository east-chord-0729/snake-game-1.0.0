...
void SnakeGame::initialize() // 게임 초기화 함수 정의
{
    ...
    createWalls(); // 가장자리에 벽을 생성합니다.
    createGate();  // 게이트를 생성합니다.
...
{
    // 뱀이 벽에 닿았는지 확인
    if (board.getCharAt(next.getY(), next.getX()) == 'O' || board.getCharAt(next.getY(), next.getX()) == 'I') 
    {
        gameOver = true; // 게임 종료
        return;
    }
...
void SnakeGame::createWalls() // 가장자리에 벽을 생성하는 함수 정의
{
    for (int i = 1; i < BOARD_ROWS-1; ++i)
    {
        board.add(Wall_G(i, 0)); // 왼쪽 벽
        board.add(Wall_G(i, BOARD_COLS - 1)); // 오른쪽 벽
    }

    for (int j = 1; j < BOARD_COLS-1; ++j)
    {
        board.add(Wall_G(0, j)); // 위쪽 벽
        board.add(Wall_G(BOARD_ROWS - 1, j)); // 아래쪽 벽
    }

    board.add(Wall_NG(0, 0)); // 왼쪽 위 모서리
    board.add(Wall_NG(0, BOARD_COLS - 1)); // 오른쪽 위 모서리
    board.add(Wall_NG(BOARD_ROWS - 1, 0)); // 왼쪽 아래 모서리
    board.add(Wall_NG(BOARD_ROWS - 1, BOARD_COLS - 1)); // 오른쪽 아래 모서리
}

void SnakeGame::createGate() // 게이트 생성 함수 정의
{
    board.add(Gate(10, 15)); // 게이트를 보드에 추가합니다.
}